...
impl Field {
    ...
    fn add_field_to_desc(
        ...
        let mut field = FieldDescriptorProto {
            ...
            json_name: Some(self.name.clone()),
            number: Some(self.index as i32),
            label: Some(if self.repeated {
        ...
        field.r#type = Some(match &self.ptype {
            ...
            _ => self.get_proto_type(&self.ptype.as_string()),
        });
        field.json_name = Some(self.name.clone());

        if self.ptype.is_nested() {
            field.type_name = Some(format!(".{}", self.ptype.as_string()));
        }
    ...
    pub fn compile_go(
        ...
        let default = if self.default.is_some() && self.default.clone().unwrap() != "PLACEHOLDER" {
...
fn import_field(field: &mut Field, object: String, tmp: String, module: &Messages) {
    ...
    for module_message in module.messages.clone() {
        if field.ptype.as_string() == module_message.name && field.ptype.is_nested() {
            let actual = field.ptype.as_string();
...
const (
	...
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
        ...
        let mut file_desc = FileDescriptorProto {
            name: Some(
                source_file
                    .to_string()
                    .replace("\\", "/")
        ...
        let mut options = FileOptions {
            go_package: Some(module.unwrap() + "/" + &*self.package.to_string()),
            ..Default::default()
        };
                ...
                if field.ptype == PType::Oneof {
                    for one_of_field in field.maybe_types.clone().unwrap().iter() {
                        if one_of_field.ptype.is_nested()
                            && !message_map.contains_key(&one_of_field.ptype.as_string())
                        {
                            message_map.insert(one_of_field.ptype.as_string(), message_map.len());
                        }
                    }
                } else if field.ptype.is_nested()
                    && !message_map.contains_key(&field.ptype.as_string())
                {
                    message_map.insert(field.ptype.as_string(), message_map.len());
                }
            }
        }
        ...
        for (i, penum) in self.penums.iter().enumerate() {
            ...
            let mut enum_desc = EnumDescriptorProto {
            ...
            for field in &penum.fields {
                ...
                let enum_val = EnumValueDescriptorProto {
